...
public class PlaybackCommand {
    ...
    public static void playNewItem(final String uri, String metadata, final Handler handler) {
        Device device = SystemManager.getInstance().getSelectedDevice();
        Log.i("JJJJ", "device:" + device.getDetails().getFriendlyName());
        //Check selected device
        if (device == null) return;
        Log.i("JJJJ", "playNewItem333:");
...
//                public void success(ActionInvocation invocation) {
                    cp.execute(new SetAVTransportURI(avtService, uri, metadata) {
                        ...
                        public void success(ActionInvocation invocation) {
                            ...
                            handler.sendEmptyMessageDelayed(IntentConstants.PLAY_MOVIE, 1200);
                        }

                        @Override
                        public void failure(ActionInvocation invocation, UpnpResponse operation, String defaultMsg) {
                            Log.e(TAG, "playNewItem failed");
//                            playNewItem(uri, handler);
                            handler.sendEmptyMessage(IntentConstants.PLAY_MOVIE_FAILED);
                        }
                    });
//                }
...
//            });
    ...
    public static void changeCodeStream(final String uri, String metadata, final Handler handler) {
        Device device = SystemManager.getInstance().getSelectedDevice();
        Log.i("JJJJ", "device:" + device.getDetails().getFriendlyName());
        //Check selected device
        if (device == null) return;
        ...
        if (avtService != null) {
            ...
            cp.execute(new Play(avtService) {
                ...
                public void success(ActionInvocation invocation) {
                    ...
                    handler.sendEmptyMessageDelayed(IntentConstants.SEEK_TO_PLAYLENGTH, 1500);
                }

                @Override
                public void failure(ActionInvocation arg0, UpnpResponse arg1, String arg2) {
                    Log.e(TAG, "Play failed");
                    handler.sendEmptyMessage(IntentConstants.PLAY_MOVIE);
                }
            });
        }
    }

    public static void playButton(final Handler handler) {
        Device device = SystemManager.getInstance().getSelectedDevice();
        //Check selected device
        if (device == null) return;
        ...
        if (avtService != null) {
            ...
            cp.execute(new Play(avtService) {
                ...
                public void success(ActionInvocation invocation) {
                    Log.i(TAG, "Play success.");
                    handler.sendEmptyMessageDelayed(IntentConstants.PLAY_ACTION, 1500);
                }
            ...
            cp.execute(new Seek(avtService, relativeTimeTarget) {
                ...
                public void success(ActionInvocation invocation) {
                    ...
                    handler.postDelayed(new Runnable() {
                        @Override
                        public void run() {
                            handler.sendEmptyMessage(IntentConstants.RESUME_SEEKBAR_ACTION);
                        }
                    }, 1000);
                }
        ...
        if (avtService != null) {
            ...
            cp.execute(new GetMediaInfo(avtService) {
                ...
                public void received(ActionInvocation invocation, MediaInfo mediaInfo) {
                    Message msg = Message.obtain(handler, IntentConstants.GET_MEDIA_INFO_ACTION);
                    msg.obj = mediaInfo;
                    msg.sendToTarget();
                }
        ...
        if (avtService != null) {
            ...
            cp.execute(new GetPositionInfo(avtService) {
                ...
                public void received(ActionInvocation invocation, PositionInfo positionInfo) {
                    Log.i("JJJJ", "positionInfo:" + positionInfo);
                    Message msg = Message.obtain(handler, IntentConstants.GET_POSITION_INFO_ACTION);
            ...
            cp.execute(new GetTransportInfo(avtService) {
                ...
                public void received(ActionInvocation invocation, TransportInfo transportInfo) {
                    ...
                    if (TransportState.PLAYING == ts) {
                        ...
                        handler.sendEmptyMessage(IntentConstants.PAUSE_ACTION);
                    } else if (TransportState.STOPPED == ts) {
                        handler.sendEmptyMessage(IntentConstants.STOP_ACTION);
                    }
                }
        ...
        if (rcService != null) {
            ...
            cp.execute(new GetVolume(rcService) {
                ...
                public void received(ActionInvocation actionInvocation, int currentVolume) {
                    ...
                    Message msg = Message.obtain(handler, IntentConstants.GET_VOLUME_ACTION, currentVolume, 0);
                    msg.sendToTarget();
                }
